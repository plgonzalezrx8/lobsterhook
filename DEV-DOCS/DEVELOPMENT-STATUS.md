...
## Current Phase
v1 implementation scaffold completed; moving into live validation and operational hardening, with sanitized webhook payload mode now available.
...
- Added HTML cleanup and Markdown fallback, plus per-account `payload_mode` support so webhook consumers can receive a slim sanitized body instead of the full normalized document.
...
- Validate minimal payload mode against real HTML-heavy mail from production senders.
...
- HTML sanitization uses heuristic cleanup rules for hidden content and quoted replies, so real mailers may still surface edge cases that need tuning.
