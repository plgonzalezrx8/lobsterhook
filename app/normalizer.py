...
def normalize_email(
    *,
    ...
    message = cast(EmailMessage, BytesParser(policy=policy.default).parsebytes(raw_path.read_bytes()))
    ...
    for part in message.walk():
        ...
        body = _read_part_content(cast(EmailMessage, part), payload)
...
def _read_part_content(part: EmailMessage, payload: bytes) -> str:
    try:
...
def _clean_text_body(value: str | None) -> str | None:
    if not value:
        return None
...
def _clean_html_body(html: str | None) -> str | None:
    ...
    # Why: remove high-confidence presentation and tracking noise while keeping
    # user-authored message content intact for downstream webhook consumers.
    # Failure mode if removed: minimal payload mode starts leaking quoted
    # threads, hidden tracking markup, and mail-client chrome.
...
def _preferred_message(
    *,
    ...
    # Why: prefer text/plain first because it usually reflects sender-authored
    # content more directly than generated HTML wrappers.
    # Failure mode if removed: webhook consumers may receive noisier HTML-derived
    # content even when a cleaner plain-text part exists.
...
def _collect_headers(message: Message[str, str]) -> dict[str, list[str]]:
...
class EmailMarkdownConverter(MarkdownConverter):
    ...
    def convert_img(self, el: Tag, text: str, parent_tags: set[str]) -> str:
        ...
        if isinstance(raw_alt, str):
            ...
            alt_source = " ".join(str(item) for item in raw_alt)
        alt = " ".join(alt_source.split())
...
def _is_hidden_tag(tag: Tag) -> bool:
    # Why: hidden DOM is usually telemetry or layout scaffolding, not the
    # message body a webhook consumer needs.
    # Failure mode if removed: minimal payloads include hidden snippets and
    # style-only artifacts that reduce signal quality.
...
def _is_tracking_image(tag: Tag) -> bool:
    # Why: 1x1 images are a common email open-tracking mechanism that carries
    # no user-visible message semantics.
    # Failure mode if removed: markdown fallback may keep tracking pixels and
    # pollute webhook payloads with irrelevant image tokens.
    if getattr(tag, "attrs", None) is None:
        return False
    if tag.name != "img":
        return False
...
def _is_quoted_reply_container(tag: Tag) -> bool:
    # Why: reply-chain containers represent previously seen history and should
    # not overshadow the newest message content in compact webhook payloads.
    # Failure mode if removed: downstream systems repeatedly ingest stale
    # conversation history and duplicate context.
    if getattr(tag, "attrs", None) is None:
        return False
    ...
    if isinstance(raw_classes, str):
        ...
        classes = " ".join(str(item) for item in raw_classes).lower()
